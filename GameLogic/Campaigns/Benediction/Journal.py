import Systems.PlayerSelect as Select


scenes = {"0": [
    ["> ", "My ship is gone. Its crew, dead."],
    ["  ", "I thought myself the only survivor until I found that witch lying unconscious on the sand."],
    ["  ", "She must have dragged herself up from the surf before succumbing to exhaustion."],
    ["  ", "I nearly threw her back in. I've shared enough of her misfortune already."],
    ["  ", "I only stayed my hand because she fought with me."],
    ["  ", "I don't know why the goddess hates her, but it's not for cowardice."],
    ["  ", "..."],
    ["  ", "At any rate, my cat survived also. She found us while I was building a fire from driftwood."],
    ["  ", "I'm glad to have at least one true ally left."],
    ["  ", "Tomorrow, we'll head north towards a village I spotted. Someone there might sell me a boat."],
    ["  ", "The witch can come along if she wakes by then."]
], "1": [
    ["> ", "So much for the village."],
    ["  ", "The town itself is rotted; none of their rowboats look salvageable."],
    ["  ", "The only thing we gained in coming here was our new companion."],
    ["  ", "..."],
    ["> ", "As for the paladin, he got stranded here after our own fashion."],
    ["  ", "His encounter with the snake occurred about a week prior, and we're the first living people he's seen since."],
    ["  ", "He won't divulge details beyond that, which suits me fine."],
    ["  ", "Whatever business brought him here, I want no further part."],
    ["  ", "..."],
    ["> ", "We're all stuck together for the moment, unfortunately."],
    ["  ", "Akeem says there's a walled town atop the cliffs to the northwest."]
    ["  ", "We'll go there next. Since we can't sail, we'll climb through the caves."]
    ["  ", "The witch should feel at home."]
]}
    # ["  ", "The man seems capable enough, but I don't like how much he smiles."],
    # ["  ", "Too much has gone awry for me to another's mirth. I wish he'd stop trying to cheer us up."],
    # ["  ", "Even Fadia seems annoyed. She won't complain though."],
    # ["  ", "I don't think that woman has it in her to say anything cruel."],
    # ["  ", "..."],
    # ["  ", "I feel bad for snapping this morning. The shipwreck wasn't her fault. Even if she is cursed."],
    # ["  ", "Akeem thinks providence brought us together. I pray he's wrong."]
  
    # ["  ", "Claims his order sent him here to investigate a nightmare shared by multiple elders."],
# 
    # They find 

    # They find an inscription stating that the lich was imprisoned by his own civilization.
    # His jailors sealed him not because he could not be destroyed, but because they wished him to suffer.